...
Creating a tenant requires selecting an already registered human account as its default administrator; the same transaction creates that person as the new workspace Owner. The tenant directory exposes governance metadata such as administrator name and email, member count, and department count, but never tenant rooms, documents, or conversations.
...
- A system administrator can create a tenant and assign an existing account as its default administrator. The transaction creates an Owner atomically, and the platform tenant directory shows the default administrator and department count without exposing tenant content.
