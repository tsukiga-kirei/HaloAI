...
- HaloAI currently has only a same-origin Web/BFF client, so a database session cookie is used. Access tokens and refresh tokens are not exposed to browser JavaScript, and credentials never enter `localStorage`. That avoids two token classes, refresh races, and XSS-readable credentials. An access/refresh protocol should be introduced behind AuthGateway only when native clients, third-party APIs, or cross-origin resource servers become real requirements.
...
- The root route `/` opens the last chosen portal. The account menu shows the current surface identity and must not keep a stale system-admin label on the collaboration UI.
