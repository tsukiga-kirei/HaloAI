...
GitHub runs two independent gates: `pnpm check` is “documentation, formatting, types, tests, and build”; browser journeys are the separate “desktop, tablet, and mobile end-to-end” check. A quiet editor is not that second check. `apps/web/next-env.d.ts` is generated by Next.js and must not be committed.
